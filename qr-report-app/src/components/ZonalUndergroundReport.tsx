...
interface SupervisorStats {
    ...
    scanned: number;
    pending: number;
    scanTiming: string; // Defaulting to 'DAY'
...
export const ZonalUndergroundReport: React.FC<ZonalUndergroundReportProps> = ({ data, date }) => {
    const reportData = useMemo(() => {
        ...
        undergroundData.forEach(record => {
            ...
            supervisorStats.records.push(record);
        });
    ...
    return (
        <div className="space-y-6">
            ...
            <div id="zonal-underground-report-container" className="bg-white p-4 min-w-[800px] overflow-x-auto">
                            ...
                            {zone.supervisors.map((sup) => {
                                ...
                                return (
                                    <React.Fragment key={sup.name}>
                                        <div className="grid grid-cols-[2fr_1fr_2fr_1fr_1fr] text-xs font-bold border-b border-black text-center items-center">
                                        ...
                                        {/* Details Row */}
                                        <div className="col-span-5 bg-gray-50 border-b border-black">
                                            <table className="w-full text-xs text-left">
                                                <thead>
                                                    <tr className="bg-gray-200 border-b border-gray-300">
                                                        <th className="p-1 border-r border-gray-300 w-10 text-center">S.No</th>
                                                        <th className="p-1 border-r border-gray-300">Site Name</th>
                                                        <th className="p-1 border-r border-gray-300">Building/Street</th>
                                                        <th className="p-1 border-r border-gray-300">Ward</th>
                                                        <th className="p-1 border-r border-gray-300">Status</th>
                                                        <th className="p-1">Scan Time</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    {sup.records.map((record, idx) => (
                                                        <tr key={record.qrId} className="border-b border-gray-100 hover:bg-gray-100">
                                                            <td className="p-1 border-r border-gray-200 text-center">{idx + 1}</td>
                                                            <td className="p-1 border-r border-gray-200">{record.siteName}</td>
                                                            <td className="p-1 border-r border-gray-200">{record.buildingName}</td>
                                                            <td className="p-1 border-r border-gray-200">{record.ward}</td>
                                                            <td className={`p-1 border-r border-gray-200 ${record.status === 'Scanned' ? 'text-green-600 font-semibold' : 'text-red-600'}`}>
                                                                {record.status}
                                                            </td>
                                                            <td className="p-1">{record.scanTime}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
